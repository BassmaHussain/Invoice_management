    ...
    () => [
      ...
      { Header: "Total amount", accessor: "total" },
      {
        ...
        Cell: ({ row }) => (
          ...
          >
            <Menu.Button
              className={`px-4 py-1 rounded text-sm ${row?.original?.status === "Paid" ? "bg-gray-200" : "bg-gray-100"}`}
            >
              {row?.original?.status}
            </Menu.Button>
            {row?.original?.status === "Paid" ? null : (
              <Menu.Items className="absolute z-50 mt-2 w-32 bg-white rounded shadow">
